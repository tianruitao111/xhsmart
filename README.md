<div align="center">

<img src="https://img.shields.io/badge/Rednote%20Spark-AI%20Content%20Platform-ff2442?style=for-the-badge&logo=data:image/svg+xml;base64,"/>

# ✨ Rednote Spark

### AI 驱动的小红书内容运营矩阵 · 从行业洞察到批量发布，全链路智能化

**AI-Powered Full-Stack Content Operations Platform for Xiaohongshu / Rednote**

[![Live Demo](https://img.shields.io/badge/🌐_立即体验-xhsmart.ltyfa.top-ff2442?style=flat-square)](https://xhsmart.ltyfa.top)
[![Free Trial](https://img.shields.io/badge/🎁_免费注册-20次生成额度-success?style=flat-square)](https://xhsmart.ltyfa.top)
[![WeChat](https://img.shields.io/badge/💬_联系微信-Tutuyunying5431-07c160?style=flat-square)](https://xhsmart.ltyfa.top)
[![Stars](https://img.shields.io/github/stars/tianruitao111/xhsmart?style=flat-square&color=ffd700)](https://github.com/tianruitao111/xhsmart/stargazers)
[![License](https://img.shields.io/badge/license-Commercial-blue?style=flat-square)](https://xhsmart.ltyfa.top)

</div>

---

## 🚀 产品简介 / Introduction

**Rednote Spark** 是一个面向小红书内容运营者的 **AI 全链路内容生产平台**，将行业洞察、选题策略、图文生成与内容排期整合在一个工作台内。

不是简单的 AI 写作工具 —— 而是一套**从市场洞察到内容落地的完整方法论与工具链**。

> 🌐 **在线体验（国内直连）**：[https://xhsmart.ltyfa.top](https://xhsmart.ltyfa.top) · 注册即送 20 次生成额度，无需信用卡

**亮点数据：**
- 🏭 支持 **13+ 热门行业**全覆盖（美妆/家居/餐饮/母婴/职场等）
- 📝 **70+ 爆款内容公式** × 10 大内容格式（Pro 专享）
- 🎨 **40+ 封面风格**，AI 智能推荐匹配
- ⚡ 无需注册即可体验游客 Demo，**注册送 20 次免费生成额度**

---

## ✨ 核心功能 / Core Features

<table>
<tr>
<td width="50%">

### 📊 行业趋势雷达
实时洞察 13+ 热门行业的内容趋势，包含：
- 用户痛点与情绪分析
- 爆款关键词挖掘
- 竞品内容策略拆解
- AI 生成行业洞察报告

**支持行业**：美妆护肤、家居装修、美食烘焙、健身运动、职场成长、母婴亲子、旅行探店、时尚穿搭、数码科技、房产置业、知识教育、宠物萌宠、情感心理

</td>
<td width="50%">

### 🎯 创作策略工坊
系统化的内容策略制定工具：
- **内容洞察**：行业热点方向库
- **产品档案**：结构化产品信息管理（13 行业专属字段）
- **客户分析**：AI 生成客户画像
- **产品案例**：成功案例素材库
- **爆款公式**：经过验证的内容结构模板

</td>
</tr>
<tr>
<td width="50%">

### 📅 智能内容日历
从策略到执行的排期系统：
- 跟随「创作策略工坊」自动同步行业 + 产品
- 一键生成月度内容计划（50条/天）
- 子主题 × 角度 × 封面风格自动分配
- 关联产品档案，生成更精准的内容

</td>
<td width="50%">

### 🎨 智能图文引擎
批量生成高质量小红书图文：
- 支持多行业、多人设并行生成
- 7种封面风格库（手绘/实拍/平面/简约等）
- AI 自动匹配标题、正文、hashtag
- 一键导出，可直接用于发布

</td>
</tr>
<tr>
<td width="50%">

### 🖼️ 封面风格库
丰富的封面设计样式：
- 7 大类 40+ 封面风格
- 行业专属封面配色方案
- AI 智能推荐匹配风格
- 实时预览效果

</td>
<td width="50%">

### 🔗 批量账号发布 *(Pro)*
多账号批量发布管理：
- 支持绑定多个小红书账号
- 内容一键分发到多账号
- 发布状态实时追踪
- 需管理员开通权限

</td>
</tr>
</table>

---

## 🎬 功能演示 / Demo Screenshots

> 💡 **[点击进入在线演示 →](https://xhsmart.ltyfa.top)**  无需注册即可体验游客模式

---

### 🏠 产品工作台 · 功能全览
![Home](https://xhsmart.ltyfa.top/demo-assets/screenshot-dashboard.png)
*一站式 AI 内容运营工作台 · 13+ 热门行业覆盖 · 无需登录即可体验游客 Demo*

---

### 🎯 创作策略工坊 · 全链路

**① 内容洞察** — 行业热点方向库，实时掌握爆款趋势
![内容洞察](https://xhsmart.ltyfa.top/demo-assets/screenshot-workshop-insight.png)

**② 选题方向** — AI 深度拆解热门赛道，提炼高转化选题角度
![选题方向](https://xhsmart.ltyfa.top/demo-assets/screenshot-workshop-topic.png)

**③ 产品档案** — 13 行业专属字段，结构化管理产品信息
![产品档案](https://xhsmart.ltyfa.top/demo-assets/screenshot-workshop-profile.png)

**④ 客户分析** — AI 一键生成精准客群画像
![客户分析](https://xhsmart.ltyfa.top/demo-assets/screenshot-workshop-customer.png)

**⑤ 爆款公式** *(Pro 专享)* — 10 大内容格式 × 70+ 子主题，付费解锁完整公式库
![爆款公式](https://xhsmart.ltyfa.top/demo-assets/screenshot-workshop-formula2.png)

---

### 🎨 封面风格库 · 40+ 精选风格
![封面风格库](https://xhsmart.ltyfa.top/demo-assets/screenshot-cover-library.png)
*手绘插画 · 实拍大片 · 平面设计 · 简约文字 · 7大类风格 · 行业专属配色方案*

---

### 🤖 智能图文引擎 · 批量生成
![智能图文引擎](https://xhsmart.ltyfa.top/demo-assets/screenshot-engine-full.png)
*多行业多人设并行 · 一键匹配标题正文 hashtag · AI 自动配图 · 批量导出可直接发布*

---

### 📅 内容日历排期
![内容日历](https://xhsmart.ltyfa.top/demo-assets/screenshot-calendar.png)
*月度内容规划 · 子主题 × 角度 × 封面自动分配 · 与策略工坊行业/产品实时联动*

---

## 💰 定价方案 / Pricing

<table>
<tr>
<th width="33%">🎁 免费注册</th>
<th width="33%">⭐ Pro 付费版</th>
<th width="33%">🏢 企业私有化部署</th>
</tr>
<tr>
<td>

- ✅ 注册赠送 **20次** 图文生成额度
- ✅ 行业趋势雷达（查看示例）
- ✅ 创作策略工坊（基础功能）
- ✅ 封面风格库预览
- ✅ 内容日历规划
- ❌ 额度用完后需升级

</td>
<td>

- ✅ **无限生成额度**（使用自己的 API Key）
- ✅ 所有基础功能 + 高级功能
- ✅ 爆款公式完整展开
- ✅ 策略蓝图 + 子主题拓展
- ✅ 产品档案完整功能
- 💬 **联系微信：Tutuyunying5431**

</td>
<td>

- ✅ 独立服务器部署
- ✅ 专属品牌定制
- ✅ 多账号批量发布功能
- ✅ 专属技术支持
- ✅ 企业账号管理
- 💬 **联系微信：Tutuyunying5431**

</td>
</tr>
</table>

---

## 🚀 快速开始 / Quick Start

### 方式一：直接使用在线版（推荐）

```
1. 访问 https://xhsmart.ltyfa.top
2. 点击右上角「注册」
3. 填写邮箱 + 密码，立即获得 20 次免费额度
4. 开始体验 AI 内容生成！
```

### 方式二：Pro 版本开通

```
1. 注册账号（获得免费额度）
2. 联系微信：Tutuyunying5431
3. 获取 Pro 权限 + 配置专属 AI API Key
4. 享受无限额度生成
```

### 方式三：企业私有化部署

```
有大量账号运营需求？需要独立数据库和品牌定制？
联系微信：Tutuyunying5431 洽谈企业部署方案
```

---

## 🛠️ 技术栈 / Tech Stack

| 层级 | 技术 |
|------|------|
| 前端 | React 18 · Ant Design 5 · Vite |
| 后端 | Node.js · Express · PostgreSQL · SQLite |
| AI 集成 | OpenAI · 豆包 · Claude · 自定义模型接口 |
| 部署 | Docker · Nginx · PM2 · Let's Encrypt SSL |

---

## 📮 联系我们 / Contact

<div align="center">

| 渠道 | 方式 |
|------|------|
| 💬 微信 | **Tutuyunying5431** |
| 🌐 在线体验 | [xhsmart.ltyfa.top](https://xhsmart.ltyfa.top) |
| ⭐ GitHub | 点个 Star 支持开发者！ |

**喜欢这个项目？欢迎 Star ⭐ 让更多人看到！**

[![Star History](https://api.star-history.com/svg?repos=tianruitao111/xhsmart&type=Date)](https://star-history.com/#tianruitao111/xhsmart&Date)

</div>

---

## ❓ 常见问题 / FAQ

**Q: 注册后能立刻使用吗？**
A: 是的，注册后立即获得 20 次免费图文生成额度，无需信用卡，无需绑定 API Key。

**Q: 免费额度用完后怎么办？**
A: 联系微信 Tutuyunying5431 开通 Pro 版，绑定自己的 API Key 后即可无限量使用。

**Q: 支持哪些 AI 模型？**
A: 支持 OpenAI (GPT-4/4o/o1)、豆包（火山引擎）、Claude (Anthropic)，以及任何兼容 OpenAI 格式的自定义模型接口。

**Q: Pro 版需要自己的 API Key 吗？**
A: 是的。Pro 版通过绑定自己的 API Key 实现无限使用，成本更低且数据更安全。

**Q: 多账号批量发布功能如何开通？**
A: 该功能属于企业版专项功能，需要管理员审核开通，联系微信 Tutuyunying5431 了解详情。

**Q: 数据安全性如何？**
A: 在线版数据存储在独立服务器，不会与第三方共享；企业版可私有化部署实现数据完全自主。

**Q: 是否支持私有化部署？**
A: 支持，联系微信洽谈企业私有化部署方案（独立服务器 + 品牌定制 + 专属支持）。

---

<div align="center">

Made with ❤️ for Rednote Content Creators

[xhsmart.ltyfa.top](https://xhsmart.ltyfa.top) · 微信：Tutuyunying5431

</div>
